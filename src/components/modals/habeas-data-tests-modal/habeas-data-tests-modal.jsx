...
import styles from './habeas-data-tests-modal.module.css'
import { setHabeasDataTestsAcceptance } from '../../../firebase/utils/user'
import { toast } from 'react-toastify'
import { DeclineModal } from '../decline-modal/decline-modal'

export function HabeasDataTestsModal({
  ...
  const handleHabeasDataTestsAcceptance = (accepted) => {
    setHabeasDataTestsAccepted(accepted)
    closeModal()
  }
  ...
  const handleAccept = useCallback(() => {
    setHabeasDataTestsAcceptance({
      userId: user.id,
      role: user.role,
    }).then(handleHabeasDataTestsAcceptance(true))
    toast.success('Ya puedes descargar tus pruebas clínicas.')
  }, [user])
